...
## 難しかった点・苦労した点
在庫移動機能の実装において、移動元と移動先の在庫数の整合性を保つ点に苦労しました。
複数のロケーション間で在庫を移動するため、
どのタイミングで在庫数を更新するかを整理しながら実装を進めました。
また、ヒアリングを進める中で要件が変化し、
初期の画面設計から変更が必要となったため、
設計と実装を繰り返し見直しながら改善しました。
